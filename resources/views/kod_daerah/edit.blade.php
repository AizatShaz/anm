                ...
                <tbody>
                  <tr>
                    ...
                    <form action="/kod_daerah/{{ $kod_daerah -> id }}" method="POST" >
                      ...
                      @csrf
                    <td>
                      <select class="form-select" name="idNegeriS" id="idNegeriS">
                        @foreach ($kod_daerah1 as $kod)
                          <option {{ $kod_daerah-> idNegeri ==  $kod-> id ? 'selected':''  }}  value="{{ $kod -> id }}">{{ $kod -> nama }}</option>
                        @endforeach
                      </select> 
                      <input value="{{ $kod_daerah -> idNegeri }}" name="idNegeri" id="idNegeriinput" type="text" class="form-control" hidden>
                    </td>
